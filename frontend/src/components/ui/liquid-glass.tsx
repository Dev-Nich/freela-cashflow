...
const GlassEffect: React.FC<GlassEffectProps> = ({
  ...
  const content = (
    <div
      className={cn(
        "relative flex cursor-pointer overflow-hidden font-semibold text-ink transition-all duration-700",
        className,
      )}
      style={glassStyle}
    >
      ...
      <div
        ...
        style={{
          boxShadow:
            "inset 1px 1px 1px 0 rgba(255, 255, 255, 0.36), inset -1px -1px 1px 1px rgba(255, 255, 255, 0.18)",
        }}
      />
  ...
  (
    ...
    const content = (
      <GlassEffect
        ...
        target={target}
        className={cn(
          "h-10 rounded-lg border border-border bg-transparent px-4 py-2 hover:bg-muted",
      ...
      >
        <div
          className={cn(
            "flex items-center justify-center gap-2 text-sm font-medium text-primary transition-all duration-300",
            className,
          )}
          style={{
            transitionTimingFunction: "cubic-bezier(0.175, 0.885, 0.32, 2.2)",
    ...
    return (
      <button
        ...
        onClick={onClick}
        className={cn(
          "inline-flex border-0 bg-transparent p-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/35 disabled:pointer-events-none disabled:opacity-50",
          wrapperClassName,
        )}
        {...props}
      >
...
export { GlassButton, GlassDock, GlassEffect, GlassFilter };
